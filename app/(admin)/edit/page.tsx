...
function    EditCourseForm() {
  ...
  const [price, setPrice] = useState<number>(Number(searchParams.get("price")) || 0);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_SERVER_URL}courses/edit-course`,
        {
          title,
          description,
          imageUrl,
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
          ...
          <form onSubmit={handleUpdate} className="space-y-5">
            ...
            {message.text && (
              <div
                className={`p-4 rounded-xl text-sm font-medium border animate-in fade-in slide-in-from-top-1 ${
                  message.isError
 ...
 * This is what Next.js sees. It wraps the form in Suspense to fix the build error.
 */
